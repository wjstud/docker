...
#    allowed_domains = ['github.com']
    ...
    def parse_info(self, response):
        ...
        item['commits'] = response.xpath('(//span[@class="num text-emphasized"])[1]/text()').re_first('\d.*')
        item['branches'] = response.xpath('(//span[@class="num text-emphasized"])[2]/text()').re_first('\d.*')
        item['releases'] = response.xpath('(//span[@class="num text-emphasized"])[3]/text()').re_first('\d.*')
        yield item
